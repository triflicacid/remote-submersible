...
typedef struct {
  const pin_t *pins[STEPPER_MOTOR_PINS]; // motor pins a, b, ...
  uint8_t step; // store current step
  uint8_t mode; // stepper motor mode
} stepper_motor_t;

// initialise a stepper motor struct
void stepper_motor_init(stepper_motor_t *motor, const pin_t *pins[STEPPER_MOTOR_PINS], uint8_t drive_mode);
