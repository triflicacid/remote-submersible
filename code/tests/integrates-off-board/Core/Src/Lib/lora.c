...
void lora_init(lora_t *lora, SPI_HandleTypeDef *spi, const pin_t *cs_pin, const pin_t *rst_pin) {
  lora->spi = spi;
  lora->cs = cs_pin;
  lora->rst = rst_pin;
...
void lora_receive(lora_t *lora, uint8_t size) {
  _write(lora, REG_DIO_MAPPING_1, 0x00); // DIO0 -> RXDONE

  if (size > 0) {
    implicit_header_mode(lora);

    _write(lora, REG_PAYLOAD_LENGTH, size);
  } else {
    explicit_header_mode(lora);
  }
