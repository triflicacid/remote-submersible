...
void lora_init(lora_t *lora, SPI_HandleTypeDef *spi, const pin_t *cs, const pin_t *rst) {
  ...
  // set base addresses
  _write(lora, REG_FIFO_TX_BASE_ADDR, FIFO_TX_BASE_ADDR);
  _write(lora, REG_FIFO_RX_BASE_ADDR, FIFO_RX_BASE_ADDR);
...
void lora_begin_packet(lora_t *lora, bool implicit_header) {
  ...
  _write(lora, REG_FIFO_ADDR_PTR, FIFO_TX_BASE_ADDR);
  _write(lora, REG_PAYLOAD_LENGTH, 0);
}

// write byte byte to FIFO
void lora_write(lora_t *lora, uint8_t byte) {
  lora_write_bytes(lora, &byte, sizeof(byte));
}

void lora_write_bytes(lora_t *lora, const uint8_t *buffer, uint8_t size) {
...
uint8_t lora_payload_length(lora_t *lora) {
  return _read(lora, REG_PAYLOAD_LENGTH);
}
...
int lora_parse_packet(lora_t *lora, int size) {
  ...
  if ((irq_flags & IRQ_RX_DONE_MASK) && !(irq_flags & IRQ_PAYLOAD_CRC_ERROR_MASK)) {
    ...
    // reset FIFO address
    _write(lora, REG_FIFO_ADDR_PTR, FIFO_RX_BASE_ADDR);
